...
import torch
import numpy as np
from models.simplex import Simplex_CLASS
...
class GaussianDiffusionModel:
    ...
    def p_mean_variance(self, model, x_t, t, estimate_noise=None):
        ...
        if estimate_noise == None:
            estimate_noise, _ = model(x_t, t).chunk(2, 1)
    ...
    def my_p_mean_variance(self, model, x_t, t, estimate_noise=None, A_funcs=None, y=None):
        """
        Finds the mean & variance from N(x_{t-1}; mu_theta(x_t,t), sigma_theta (x_t,t))
        ...
        pred_x_0 = self.predict_x_0_from_eps(x_t, t, estimate_noise).clamp(-1, 1)
        pred_x0_t_hat = pred_x_0 - A_funcs.A_pinv(
            A_funcs.A(pred_x_0.reshape(pred_x_0.size(0), -1)) - y.reshape(y.size(0), -1)
        ).reshape(*pred_x_0.size())
        model_mean, _, _ = self.q_posterior_mean_variance(
                pred_x0_t_hat, x_t, t
                )
        return {
    ...
    def my_sample_p(self, model, x_t, t, denoise_fn="gauss", A_funcs=None, y=None):
        out = self.my_p_mean_variance(model, x_t, t, A_funcs=A_funcs, y=y)
        # noise = torch.randn_like(x_t)
        if type(denoise_fn) == str:
            if denoise_fn == "gauss":
                noise = torch.randn_like(x_t)
    ...
    def forward_backward(
            self, model, x, see_whole_sequence="half", t_distance=None, denoise_fn="gauss", A_funcs=None, y=None
            ):
        ...
        if see_whole_sequence == "whole":
            ...
            # t_tensor = torch.full((x.shape[0],), t_distance - 1, device=x.device, dtype=torch.long)
            t_tensor = torch.tensor([t_distance - 1], device=x.device, dtype=torch.long).repeat(x.shape[0])
            x = self.sample_q(
                    x, t_tensor,
        ...
        for t in range(int(t_distance) - 1, -1, -1):
            t_batch = torch.tensor([t], device=x.device).repeat(x.shape[0])
            with torch.no_grad():
                out = self.my_sample_p(model, x, t_batch, denoise_fn, A_funcs, y)
                x = out["sample"]
    ...
    def calc_total_vlb(self, x_0, model, args):
        ...
        total_vlb = vlb.sum(dim=1) + prior_vlb
        return {
            "total_vlb": total_vlb,
            "prior_vlb": prior_vlb,
            "vlb":        vlb,
