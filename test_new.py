...
def test(args, model, diffusion, test_loader):
    ...
    for (data, label, mask) in tqdm(test_loader):
        ...
        with torch.no_grad():
            ...
            score = msgms(output, data, as_loss=False)
            out_mask_gradient = mean_smoothing(score, 21)
            #out_mask_gradient = mgsgmsmapmean.detach().cpu().numpy()
            recimg = output.detach().cpu().numpy()[0]
            recimg = np.transpose(recimg, (1, 2, 0)) * 180
            recimg = recimg.astype('uint8')
            oriimg = data.detach().cpu().numpy()[0]
        ...
        # combined
        score = colorD[None, None, :, :] + out_mask_gradient
...
def parse_args():
    ...
    parser.add_argument('--noise_fn', type=str, default='simplex', help='gauss, simplex_randParam, random, simplex')
