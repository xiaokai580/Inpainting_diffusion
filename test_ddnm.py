...
from models.GaussianDiffusion_ddnm import GaussianDiffusionModel, get_beta_schedule
...
def test(args, model, diffusion, test_loader):
    ...
    # 制作掩膜
    from utils.funcs import Inpainting, gen_mask
    k_value = random.sample([4, 8, 12], 1)
    ...
    for (data, label, mask) in tqdm(test_loader):
        ...
        with torch.no_grad():
            ...
            outputs = []
            for M in Ms:
                mask = torch.from_numpy(M).to(device).reshape(-1)
                missing_r = torch.nonzero(mask == 0).long().reshape(-1) * 3
                missing_g = missing_r + 1
                missing_b = missing_g + 1
                missing = torch.cat([missing_r, missing_g, missing_b], dim=0)
                A_funcs = Inpainting(args.in_channels, args.img_size, missing, device)
                y = A_funcs.A(data)

                m = diffusion.forward_backward(model, data, see_whole_sequence=None, t_distance=args.sample_distance // 2, A_funcs=A_funcs, y=y)
...
def main(args):
    ...
    plt.legend(loc="lower right")
    save_dir = args.save_dir + '/' + 'test_ddnm/' + 'pictures_{:.4f}'.format(threshold)
...
def parse_args():
    ...
    parser.add_argument('--sample_distance', type=int, default=200)
